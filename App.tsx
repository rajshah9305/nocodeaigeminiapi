...
const App: React.FC = () => {
  ...
  const handleSendMessage = async (promptText: string) => {
    ...
    const currentChatMode = messages.length <= 1 ? 'builder' : chatMode;

    if (currentChatMode === 'builder') {
  ...
  const handleBuilderMessage = async (text: string) => {
     setActiveSources([]);
     try {
      const { code: generatedCode, sources } = await generateAppCode(text);
  ...
  const handleRegenerate = (prompt: string) => {
    ...
    setIsLoading(true);
    setError(null);
    handleBuilderMessage(prompt).finally(() => setIsLoading(false));
  }
  ...
  const handleDownload = async () => {
    const zip = new JSZip();
    zip.file('index.html', activeCode.html);
    zip.file('style.css', activeCode.css);
    zip.file('script.js', activeCode.javascript);
    
    const content = await zip.generateAsync({ type: 'blob' });
    saveAs(content, 'raj-ai-app.zip');
  };
  ...
  const handleMouseMoveVertical = (e: MouseEvent) => {
    if (isResizingVertical.current && mainPanelRef.current) {
       const mainRect = mainPanelRef.current.getBoundingClientRect();
       const newHeight = window.innerHeight - e.clientY - (window.innerHeight - mainRect.bottom);
  ...
  if (isInitialView) {
    return (
      <div className="w-full h-screen font-sans flex flex-col items-center justify-center animate-fade-in p-6 bg-gray-100">
          <div className="w-full max-w-3xl text-center">
              <BrainCircuit className="h-16 w-16 mx-auto text-primary mb-4" />
              <h1 className="text-4xl font-bold text-gray-900 mb-2 tracking-tight">RAJ AI App Builder</h1>
              <p className="text-lg text-gray-600 mb-8">Transform your ideas into web applications with natural language.</p>
              ...
              <div className="bg-white rounded-xl p-4 shadow-xl">
                  ...
                  <div className="relative">
                      <textarea
                        ...
                        placeholder="e.g., 'a Pomodoro timer with a start, stop, and reset button'"
                        className="w-full p-4 pr-16 bg-white border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/80 transition-all resize-none text-base shadow-inner"
                        disabled={isLoading}
                        rows={2}
                      ...
                      <button
                        ...
                        className="absolute right-3 top-1/2 -translate-y-1/2 bg-primary text-white h-10 w-10 flex items-center justify-center rounded-lg hover:bg-primary-dark disabled:bg-gray-300 disabled:cursor-not-allowed transition-colors"
  ...
  return (
    <div className="main-container font-sans text-gray-800 bg-gray-100">
      <Header onDownload={handleDownload} />
      <div className="flex-1 flex flex-col p-4 pt-0 gap-4">
        <main className="flex flex-1 overflow-hidden" ref={mainPanelRef}>
          <div style={{ width: `${panelWidth}px` }} className="overflow-hidden h-full flex-shrink-0 panel bg-white">
            <SidePanel 
              code={activeCode} 
          ...
          <div className="flex-1 flex flex-col h-full overflow-hidden panel bg-white">
          ...
          <div className="p-4 bg-white/80 backdrop-blur-sm border-t border-gray-200 flex-shrink-0">
            ...
            <div className="relative max-w-4xl mx-auto">
              ...
              <textarea
                ...
                placeholder={chatMode === 'builder' ? 'Describe a change or a new app...' : 'Ask the AI assistant anything...'}
